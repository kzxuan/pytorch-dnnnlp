...
import deep_neural.pytorch as dnpt


class RNN_diachronic_model(nn.Module, dnpt.base):
    def __init__(self, n_time, args):
        nn.Module.__init__(self)
        dnpt.base.__init__(self, args)
        ...
        for _ in range(n_time):
            self.extractors.append(
                dnpt.LSTM_layer(self.emb_dim, self.n_hidden, self.n_layer, self.drop_prob,
                                self.bi_direction, self.GRU_enable, self.use_attention)
            )  # index 0 -> (nt-1)
            self.predictors.append(
                nn.Sequential(
                    nn.Linear(out_n_hidden, self.n_class),
                    nn.Softmax(-1)
                )
            )  # index 0 -> (nt-1)
