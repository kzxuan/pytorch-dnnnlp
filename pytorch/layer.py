...
class CNN_layer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_width, input_size, stride=1):
        """
        CNN layer
        ...
        * input_size [int]: embedding dim or the last dim of the input for the second dim of kernel
...
class LSTM_layer(nn.Module):
    ...
    def forward(self, inputs, seq_len, out_type='all'):
        ...
        * seq_len [tensor]: sequence length (batch_size,)
        * out_type [str]: use 'last'/'all' to choose
        ...
        if out_type == 'all':
            ...
            raise ValueError("! Wrong value of parameter 'out-type', accepts 'last'/'all' only.")
